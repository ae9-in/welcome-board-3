...
export function CompetitionsGrid({ competitions }: { competitions: Competition[] }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight") setOpenIdx((i) => (i === null ? null : (i + 1) % competitions.length));
      if (e.key === "ArrowLeft") setOpenIdx((i) => (i === null ? null : (i - 1 + competitions.length) % competitions.length));
    };
        ...
        {active && openIdx !== null && (
          ...
          >
            <motion.div
              ...
              initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.05 } }} exit={{ opacity: 0 }}
            />
            <motion.div
              role="dialog"
            ...
            >
              ...
              >
                ...
                <button onClick={() => setOpenIdx(null)} aria-label="Close" className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-black/35 text-white backdrop-blur-md transition hover:bg-black/50">
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div className="flex-1 overflow-auto px-7 py-9 md:px-10 md:py-11">
                <h3 className="font-display text-3xl font-semibold tracking-tight md:text-4xl">{active.title}</h3>
                <p className="mt-6 text-lg leading-[1.85] text-foreground/80 md:text-xl">{active.full_description}</p>
              </div>
              <div className="flex items-center justify-between border-t border-border px-6 py-4">
                <button onClick={() => setOpenIdx((openIdx - 1 + competitions.length) % competitions.length)} className="flex items-center gap-2 text-sm text-foreground/70 hover:text-foreground">
                  <ChevronLeft className="h-4 w-4" /> Previous
                </button>
                <span className="font-mono-meta text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Use ← → to navigate</span>
                <button onClick={() => setOpenIdx((openIdx + 1) % competitions.length)} className="flex items-center gap-2 text-sm text-foreground/70 hover:text-foreground">
